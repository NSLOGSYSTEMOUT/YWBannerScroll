...
@interface BannerRollingView ()<UIScrollViewDelegate>{

    //手动拖拽是保存的偏移量，一遍确定判断方向
    CGFloat preOffsetX;
}
...
@implementation BannerRollingView
...
-(UIScrollView *)scrollView{
    ...
    if (!_scrollView) {
        ...
        self.pageCount = [self.dateSource countOfCellForBannerRollingView:self];
        ...
        for (int i = 0; i < _pageCount + 2; i++)
        {
            ...
            {
                // 最后多余的一页跟第一页一样并重新定义
                pageView = [self.dateSource bannerRollingView:self cellAtIndex:0];
            }
            else
            {
                pageView = [self.dateSource bannerRollingView:self cellAtIndex:i - 1];
            }
            // 添加pageview
